...
export default class TrangChu extends Component<Props> {
  ...
  loadDanhSachBanner() {
    ...
    fetch('http://dev.baohanhdientu.net/api/Setting_API/GetListByUserID?Type=2', {method: "GET"}).then(response => {
            ...
            //Alert.alert(responseJson);
             ...
             responseJson.forEach(function(element) {
                  banners.push({
                     title: element.Title,
                     caption: null,
                     url: "http://dev.baohanhdientu.net/Images/App/" + element.ImagePath
                   });
              });
              ...
              this.setState({interval: setInterval(() => {
                  this.setState({
                    position: this.state.position === banners.length
                      ? 0
  ...
  render() {
    return (<Container>
      ...
      <Footer style={styles.footer_home}>
        ...
        <View style={[styles.flexrow, styles.footer_home_row]}>
          <TouchableOpacity style={[styles.footer_home__item]} onPress={()=> this.navToDiemTichLuy()}>
            <Text style={styles.footer_home__item__icon}>
          ...
          <TouchableOpacity style={[styles.footer_home__item]} onPress={()=> this.navToDoanhThu()}>
            <Text style={styles.footer_home__item__icon}>
              <FontAwesome name='usd' size={30}/>
            </Text>
            <Text style={styles.footer_home__item__text}>Doanh thu</Text>
          </TouchableOpacity>
